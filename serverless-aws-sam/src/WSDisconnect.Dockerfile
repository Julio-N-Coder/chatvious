...
WORKDIR /app/src/websocket-endpoints/disconnect
COPY ${function_directory}/package*.json ./
RUN npm install
COPY ${function_directory}/disconnect.ts ${function_directory}/tsconfig.json ./
RUN npm run build
...
WORKDIR ${LAMBDA_TASK_ROOT}/websocket-endpoints/disconnect
COPY ${function_directory}/package*.json ./
RUN npm install

CMD ["websocket-endpoints/disconnect/disconnect.handler"]