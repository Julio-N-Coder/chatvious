...
impl TokensClient {
    pub async fn new() -> Self {
        TokensClient {
            ...
            ssm_client: Self::create_ssm_client().await,
        }
    }
    ...
    async fn retrieve_key_string(&self, key_name: &str) -> Result<String, aws_sdk_ssm::Error> {
        self.get_parameter(key_name, false).await
    }

    async fn retrieve_private_key(&self) -> Result<EncodingKey, TokenVerificationError> {
        let pem_string = self
            .retrieve_key_string("/chatvious/private_key")
            .await
            .map_err(TokenVerificationError::SsmError)?;

        // PEM parsing for ED25519
        let encoding_key = EncodingKey::from_ed_pem(pem_string.as_bytes())
            .map_err(TokenVerificationError::PemParsingError)?;
        Ok(encoding_key)
    }

    async fn retrieve_public_key(&self) -> Result<DecodingKey, TokenVerificationError> {
        let pem_string = self
            .retrieve_key_string("/chatvious/public_key")
            .await
            .map_err(TokenVerificationError::SsmError)?;
    ...
    pub async fn verify_refresh_token(
        ...
        self.verify_token::<RefreshTokenClaims>(token, "refresh")
            .await
    }
    ...
    {
        let decoding_key = self.retrieve_public_key().await?;
    ...
    async fn generate_token_set_base(
        ...
        let encoding_key = self.retrieve_private_key().await?;
