ARG function_directory="./roomRoutes/deleteRoom"
...
WORKDIR /app/src/roomRoutes/deleteRoom
COPY ${function_directory}/package*.json ./
RUN npm install
COPY ${function_directory}/deleteRoom.ts ${function_directory}/tsconfig.json ./
RUN npm run build
...
WORKDIR ${LAMBDA_TASK_ROOT}/roomRoutes/deleteRoom
COPY ${function_directory}/package*.json ./
RUN npm install

CMD ["roomRoutes/deleteRoom/deleteRoom.handler"]