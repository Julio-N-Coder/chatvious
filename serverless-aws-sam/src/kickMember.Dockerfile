ARG function_directory="./roomRoutes/kickMember"
...
WORKDIR /app/src/roomRoutes/kickMember
COPY ${function_directory}/package*.json ./
RUN npm install
COPY ${function_directory}/kickMember.ts ${function_directory}/tsconfig.json ./
RUN npm run build
...
WORKDIR ${LAMBDA_TASK_ROOT}/roomRoutes/kickMember
COPY ${function_directory}/package*.json ./
RUN npm install

CMD ["roomRoutes/kickMember/kickMember.handler"]
