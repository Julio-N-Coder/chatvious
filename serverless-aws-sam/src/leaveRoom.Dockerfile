ARG function_directory="./roomRoutes/leaveRoom"
...
WORKDIR /app/src/roomRoutes/leaveRoom
COPY ${function_directory}/package*.json ./
RUN npm install
COPY ${function_directory}/leaveRoom.ts ${function_directory}/tsconfig.json ./
RUN npm run build
...
WORKDIR ${LAMBDA_TASK_ROOT}/roomRoutes/leaveRoom
COPY ${function_directory}/package*.json ./
RUN npm install

CMD ["roomRoutes/leaveRoom/leaveRoom.handler"]