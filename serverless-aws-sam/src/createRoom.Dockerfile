ARG function_directory="./roomRoutes/createRoom"
...
WORKDIR /app/src/roomRoutes/createRoom
COPY ${function_directory}/package*.json ./
RUN npm install
COPY ${function_directory}/createRoom.ts ${function_directory}/tsconfig.json ./
RUN npm run build
...
WORKDIR ${LAMBDA_TASK_ROOT}/roomRoutes/createRoom
COPY ${function_directory}/package*.json ./
RUN npm install

CMD ["roomRoutes/createRoom/createRoom.handler"]
